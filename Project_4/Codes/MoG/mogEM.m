...
function [p,mu,vary,logProbX] = mogEM(x,K,iters,minVary,plotFlag, randConst)
...
for i=1:iters
 ...
 % [mx, ~] = max(logPcAndx,[],1); 
  mx = sum(exp(logPcAndx), 1);
  mx = log(mx);
  
  PcAndx = exp(logPcAndx - ones(K,1)*mx); %%
  Px = sum(PcAndx,1);
  PcGivenx = PcAndx./(ones(K,1)*Px); 
  logProb = log(Px) + mx;  %%
  ...
  if plotFlag     % Plot the data and Gaussians
    ...
    hold on;
    for k=1:K
      plotEllipse(mu(1,k),mu(2,k),vary(1,k),vary(2,k),0,ellColor(mod(k, ...
          length(ellColor))+1));
    end;
    for t=1:T  plot(x(1,t),x(2,t),char(plotString(mod(mxi(t), ...
            length(plotString))+1))); end
  ...
  for k=1:K
    ...
    respDist(:,k) = mean((x-mu(:,k)*ones(1,T)).^2.*(ones(N,1)*...
        PcGivenx(k,:)),2);
  end;
